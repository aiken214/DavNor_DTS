...
<body class="bg-gray-100 flex items-center justify-center min-h-screen p-4 font-sans selection:bg-none">
    ...
    <script>
        ...
        async function joinQueue(type) {
            ...
            const res = await fetch(`{{ route('queue.api.join') }}?type=${type}`, { method: 'POST' });
        ...
        async function updateLiveStatus() {
            ...
            try {
                const res = await fetch(`{{ route('queue.api.ticket-status') }}?id=${myTicketId}`);
                const data = await res.json();
        ...
        async function setupPushNotifications(ticketId) {
            ...
            try {
                ...
                await fetch('{{ route("queue.api.save-subscription") }}', {
                    method: 'POST',
