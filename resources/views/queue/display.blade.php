...
<body class="bg-gray-100 text-gray-800 min-h-screen flex flex-col font-sans overflow-hidden select-none">
    ...
    <script>
        ...
        function formatNextList(arr, prefix) {
        ...
        async function updateTV() {
            try {
                const res = await fetch('{{ route("queue.api.status") }}');
                if (!res.ok) return;
                ...
                const recActive = data.recActive;
                ...
                if (recActive) {
                    recNumContainer.innerText = 'REC-' + pad(recActive);
                ...
                if (relActive) {
                    relNumContainer.innerText = 'REL-' + pad(relActive);
                ...
                // Up Next lists from JSON
                document.getElementById('rec-next').innerText = formatNextList(data.recNext, 'REC');
                document.getElementById('rel-next').innerText = formatNextList(data.relNext, 'REL');
