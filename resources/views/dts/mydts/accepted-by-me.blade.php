...
<div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
    <h6 class="fw-semibold mb-0">Accepted by Me</h6>
...
<div class="card basic-data-table">
    <div class="card-header">
        <h5 class="card-title mb-0">Documents Accepted by Me</h5>
    </div>
    <div class="card-body">
        <table id="acceptedByMeTable" class="table table-striped">
            <thead>
                <tr>
                    <th style="text-align: left;">Tracking</th>
                    <th>Particulars</th>
                    <th>From</th>
                    <th>Accepted Section</th>
                    <th>Date Accepted</th>
                    <th>Status</th>
                </tr>
            </thead>
            <tbody>
                @foreach($documents as $route)
                    <tr>
                        ...
                        <td>@dateDateTime($route->date_accepted)</td>
                        <td>
                            @if($route->status_id == 2)
                                <span class="badge bg-success">Pending Action</span>
                            @elseif($route->status_id == 3)
                                <span class="badge bg-info">Filed/Kept</span>
                            @elseif($route->status_id == 5)
                                <span class="badge bg-warning">Deferred</span>
                            @elseif($route->status_id == 6)
                                <span class="badge bg-secondary">Forwarded</span>
                            @else
                                <span class="badge bg-secondary">Status {{ $route->status_id }}</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
